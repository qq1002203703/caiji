...
<div class="fly-panel fly-panel-user" pad20>
        ...
        <div class="layui-tab-content">
            ...
            <div class="layui-form">
                <p style="font-size:13px;">
                    <a class="layui-btn" href="<?=url('admin/portal/post_add')?>?type=<?=$type?>">发布<?=$cs['pindao'][$type]?></a>&nbsp;&nbsp;&nbsp;
                    <a href="<?=url('admin/portal/post')?>?type=<?=$type?>&status=1">已发布(<?=$total_fabu?>)</a>
                    | <a href="<?=url('admin/portal/post')?>?type=<?=$type?>&status=3">定时发布(<?=$total_dingshi?>)</a>
                </p>
                <form method="post" action="<?=url('admin/api_portal/post_multi_action')?>?type=<?=$type?>" name="form2" id="form2">
                ...
                <form class="layui-form" name="form1" id="form1" method="get" action="" lay-filter="shaixuan">
                    ...
                    <table class="layui-table">
                        ...
                        <tbody id='find_ckeck'>
                        <?php /*dump($data);*/ if($data): $thumbFormat=getThumbFormat();foreach ($data as $item): ?>
                            <tr id="c<?=$item['id']?>">
                                <td>
                                ...
                                <td>
                                    <?=$item['id']?>.&nbsp;<a href="<?=url('admin/portal/post_edit',['id'=>$item['id'],'type'=>$type])?>" title="编辑"><?=$item['title']?></a>
                                    <a href="<?=url('@'.$item['type'].'@',['id'=>$item['id']])?>" class="gray yang-icon"  target="_blank" title="去前台浏览"><i class="layui-icon layui-icon-link"></i></a>
                                </td>
                                <td align="center"><img src="<?=getThumb($item['thumb'],'')?>" style="max-height: 80px;"></td>
                                <td><?=$item['username']?></td>
                                ...
                                <td style="text-align: center;">
                                    <i class="layui-icon" style="font-size: 20px;">
                                        <a title="修改" href="<?=url('admin/portal/post_edit',['id'=>$item['id'],'type'=>$type])?>">&#xe642;</a>
                                    </i>
                                    <i class="layui-icon" style="font-size: 20px;">
                                        <a title="删除" id="deletea_<?=$item['id']?>" href="javascript:;" ac="<?=url('admin/api_portal/post_del')?>?type=<?=$type?>&id=<?=$item['id']?>">
                                            &#xe640;
                                        </a>
                                    </i>
                                    <i class="layui-icon" style="font-size: 20px;">
                                        <a title="复制本文新发一篇" href="<?=url('admin/portal/post_copy')?>?type=<?=$type?>&id=<?=$post['id']?>">&#xe60a;</a>
                                    </i>
                                </td>
                            </tr>
    ...
    <script type="text/javascript">
        var navSidebar="portal-post-<?=$type?>";
    ...
    <script type="text/javascript">
        layui.use(['layer','fly','form'], function() {
            ...
            $("#shaixuan").click(function () {
               ...
               window.location.href="<?=url('admin/portal/post')?>?type=<?=$type?>&status=<?=get('status','int',1)?>"+url;
            })
        });
