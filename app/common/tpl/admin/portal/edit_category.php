				...
				<div class="pure-u-3-4">
					<form class="pure-form pure-form-stacked" action="" method="post">
						 ...
						 <div class="tab-item" id="category-main">
							...
							<input id="name" type="text" value="<?=$category['name']?>" name="name" placeholder="分类名" required>
							<label for="slug">分类别名（slug）</label>
							<input id="slug" type="text" name="slug" placeholder="分类别名" value="<?=$category['slug']?>" >
							<label for="description">描述（最多255个字）</label>
							<textarea id="description" name="description" class="pure-input-1-2"><?=$category['description']?></textarea>
							</div>
							 <div class="tab-item" id="category-seo">
								<label for="seo_title">SEO标题（最多100个字）</label>
								<input class="pure-input-1-2" value="<?=$category['seo_title']?>" id="seo_title" type="text" name="seo_title" placeholder="SEO标题">
								<label for="seo_keywords">SEO关键词(多个关键词用英文逗号分隔)</label>
								<input class="pure-input-1-2"  value="<?=$category['seo_keywords']?>" id="seo_keywords" type="text" name="seo_keywords" placeholder="SEO关键词">
								<label for="seo_description">SEO描述（最多255个字）</label>
								<input class="pure-input-1-2" value="<?=$category['seo_description']?>" id="seo_description" type="text" name="seo_description" placeholder="SEO描述">
							 </div>
							 <div class="tab-item" id="category-tpl">
								<label for="list_tpl">列表模板</label>
								<input class="pure-input-1-2" value="<?=$category['list_tpl']?>" id="list_tpl" type="text" name="list_tpl" placeholder="列表模板">
								<label for="one_tpl">文章模板</label>
								<input class="pure-input-1-2" value="<?=$category['one_tpl']?>" id="one_tpl" type="text" name="one_tpl" placeholder="文章模板">
							 </div>
