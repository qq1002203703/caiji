...
class Weibo extends Spider
{
    ...
    public function test(){
        ...
        //我是一个,我这个人,我这样的人,自己是一个,自我介绍,介绍一下自己,关于我自己
        ...
        for ($i=10;$i<150;$i++){
            ...
            if($i==1){
                if(!isset($res['data']['cards'][3]['card_group'])|| !$res['data']['cards'][3]['card_group']){
                    dump($res);
                    //$this->dodo($res);
                    exit($this->error[-4].PHP_EOL);
                }
                $res=$res['data']['cards'][3]['card_group'];
            }else{
...
//        exit();
            ...
            foreach ($res as $item){
                ...
                if(!$this->check($item['mblog']['text'])){
                    echo '  字数不符合要求跨过'.PHP_EOL;
                    continue;
                }
                if(!$this->saveText(['text'=>$item['mblog']['text']])){
                    $isFalse=true;
                }
            }
            if($isFalse)
    ...
    protected function check($text){
        if(strpos($text,'嘉宾')!==false)
            return false;
        $length=mb_strlen(strip_tags($text));
        return ($length>=54 && $length<=500);
    }
    protected function saveText($data){
        $data['md5']=md5($data['text']);
        $table='caiji_renren_name';
        if($this->model->from($table)->eq('md5',$data['md5'])->find(null,true)){
            echo '  重复：----------- '.PHP_EOL;
            return true;
        }else{
            $tmp=$this->model->select('id')->from($table)->eq('md5','')->gt('id',12000)->find(null,true);
            if(!$tmp)
                exit('  没有需要更新的了'.PHP_EOL);
            if($this->model->from($table)->eq('id',$tmp['id'])->update($data)){
                echo '  更新：'.$tmp['id'].'---'.$data['text'].'----------- '.PHP_EOL;
                return true;
            }
            return false;
        }
