...
Simditor = (function(superClass) {
  ...
  Simditor.prototype.sync = function() {
    ...
    val = $.trim(cloneBody.html().replace(/<p(.*?) style=""(.*?)>/ig, "<p$1$2>").replace(/<br><\/([a-z0-9]+?)>/ig,'</$1>'));
    this.textarea.val(val);
    return val;
  };
...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root['SimditorHTML'] = factory(jQuery,Simditor);
  }
}(this, function ($, Simditor) {
...
HTMLButton = (function(superClass) {
  ...
  HTMLButton.prototype.beautifyHTML = function(html) {
	  return html.replace(/<\/([a-z0-9]+?)>/ig,"</$1>\n");
    /*var args;
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    if (beautify.html) {
