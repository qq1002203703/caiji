...
<?php if($data):?>
    <ul class="yang-list">
        <?php  foreach ($data as $article):?>
            <li>
                <a class="w75 grid mb2 mt2 clearfix pr2 pl2" href="<?=url('@goods@',['id'=>$article['id']])?>">
                    <div class="box col-8">
                        <h3 class="f34"><?=$article['title']?></h3>
                    </div>
                    <div class="box col-4"><img <?=($article['thumb'] ?'src="'.$article['thumb'].'" alt="'.$article['title'].'"' : 'src="/uploads/images/no.gif" alt="没有缩略图"')?>></div>
                </a>
            </li>
        <?php endforeach;?>
    </ul>
