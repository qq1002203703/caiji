...
class Image
{
    ...
    protected $width = 0; // 要设置的宽度
    protected $height= 0; // 要设置的高度
    protected $create_thumb = ''; // 是否创建缩略图
    protected $new_name='';//修改后图片名称，为空时覆盖原图
    protected $thumb_marker = ''; // 缩略图后缀
    protected $marktext = 'www.dahulu.cc';
    ...
    private function imagetext($image,$args = null) {
        $text = isset($args['text']) && $args['text'] ?  : $this->marktext;
        $font = isset($args['fontfamily']) && $args['fontfamily'] ? $this->fontfamily[$args['fontfamily']] : $this->fontfamily[0] ;
        $fontsize = isset($args['fontsize']) && $args['fontsize'] ? (int) $args['fontsize'] : $this->fontsize ;
        ...
        imagettftext($image, $fontsize, 0, $this->width - $tb[2] - 4, $this->height - 5, $grey, $font, $text);
        // Add the text
        imagettftext($image, $fontsize, 0, $this->width - $tb[2] - 5, $this->height - 5, $white, $font, $text);
        //imagepng($this->image);
        return $image;
    }
}